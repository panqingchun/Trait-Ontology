...
public function get_ajax_crmname(){
       ...
       $map = "trait like '$demo3%'";
       $result = $Article->distinct(true)->field('Trait')->where($map)->select(); 
       foreach($result as $row){
                $tra[] = array_values($row);
              }
       $this->ajaxReturn($tra);
}
public function info(){
      ...
      public function search_trait(){
              $species = $_POST['species'];
              $traitid=$_POST['keyword'];
              // if($species=="none" || $traitid=="none"){
              //   $a ="none";
              //   $this->ajaxReturn($a);
              // }
               if($species=="Maize"){
                ...
                $where['Trait']=$traitid;
               $tra=$maize->field('ID,ID_ref,Marker_name,chr_ref,Marker_location,Trait,allele,organ,Pvalue,indel_snp,MAF,Pop_type,Pop_size,Marker_set,Model,Genome_version,final_version,Chr,Position,Ref,note,Ref_name,Chr_gene,Start,Gene_refGene,New_gene,Annotation,Level1,Level2,Level3,Level4,Level5,Level6,GO_ID')->where($where)->select();

               session('bbb',$tra);
                // $tra=$maize->distinct(true)->field('Gene_refGene,Trait,Level1,Level2,Level3,Level4,Level5,Level6')->where($trait)->select();
                $this->ajaxReturn($tra);
