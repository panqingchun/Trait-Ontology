    ...
    $(".p_example").click(function(){
        ...
        if($("#species").val()=="Rice"){
            ...
            $("#submit").click(function(){
                var geneList = $("#geneid").val().split(/,|，|\s/);
                if($("#species").val() === "Maize") {
                    var hasV4 = false;
                    var hasV3 = false;
                    geneList.forEach(function(el, index, arr) {
            ...
            function findInfo( geneList ) {
                    loading('Search genes');
                    $("#tbody").empty();
                $.post("<?php echo U('Genes/printgene');?>",{gene:geneList,select:$("#species").val()},
                function(data){
                    hiding();
                  if(data==""){
